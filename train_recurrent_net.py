...
if __name__ == '__main__':
    ...
    model = RecurrentNet(sents_padded_train.size(1), len(
        vocabulary) + 1, 500, config.NUM_LAYERS, 800, 1, dropout=config.DROPOUT)
    ...
    for epoch in range(config.NUM_EPOCHS):
        ...
        hidden_layer = model.init_hidden(config.BATCH_SIZE)
        for i, (local_batch, local_labels) in enumerate(training_generator):
            # skip the batch if it contains less elements than batch size
            if local_batch.size(0) != config.BATCH_SIZE:
                continue
            optimizer.zero_grad()
            ...
            # binary cross entropy requires size of [batch_size, 1] for labels
