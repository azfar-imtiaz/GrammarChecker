...
def generate_training_data(sent_pairs, voc):
    sent_pairs = sorted(sent_pairs, key=lambda x: len(
        x[0].split()), reverse=True)
...
if __name__ == '__main__':
    ...
    vocabulary, sent_pairs = prepare_training_data(dataset[:10])
    input_stuff, output_stuff = generate_training_data(sent_pairs, vocabulary)
    ...
    decoder = DecoderRNN(embedding, hidden_size=decoder_hidden_size, output_size=vocabulary.num_words, num_layers=2)
    criterion = NLLLoss(ignore_index=vocabulary.PAD_TOKEN)
