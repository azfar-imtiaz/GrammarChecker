...
def get_numeric_representation_sentence(sent, voc):
    ...
    numeric_sent = [voc.word2index[w] for w in word_tokenize(sent)]
...
def pad_sents(batch_sents, fill_value):
    padded_sents = list(itertools.zip_longest(*batch_sents, fillvalue=fill_value))
    return padded_sents
...
def generate_binary_matrix(batch_numeric_sents, fill_value):
    ...
    for index, seq in enumerate(batch_numeric_sents):
        temp_row = []
        for token in seq:
            temp_row.append(1) if token != fill_value else temp_row.append(fill_value)
...
def get_padded_sequences_output(sents_batch, voc):
    numeric_sents_batch = [get_numeric_representation_sentence(s, voc) for s in sents_batch]
    max_seq_length = max([len(s) for s in numeric_sents_batch])
    padded_sequences = pad_sents(numeric_sents_batch, fill_value=voc.PAD_TOKEN)
    binary_mask = generate_binary_matrix(padded_sequences, fill_value=voc.PAD_TOKEN)
